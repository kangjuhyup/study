...
def main():
    ...
    meta={'started_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'model_requested':'gpt-5.6-sol','reasoning_requested':'medium','codex':subprocess.check_output(['codex','--version'],text=True).strip(),'python':subprocess.check_output(['python3','--version'],text=True).strip(),'order':order,'seed':args.seed,'command':cmd,'fixture_sha256':hashes(ROOT/'fixture'),'prompts_sha256':hashes(ROOT/'prompts'),'grader_sha256':hashlib.sha256((ROOT/'grade.py').read_bytes()).hexdigest()}
    ...
    with tempfile.TemporaryDirectory(prefix='study-workflow-code-') as td:
        ...
        for i,(block,arm) in enumerate(order,1):
            ...
            shutil.copytree(ROOT/'fixture',work);initial=hashes(work)
            ...
            try:
                grader=subprocess.run(['python3','-I',str(ROOT/'grade.py'),str(work/'tags.py')],text=True,capture_output=True,env=env,timeout=15)
