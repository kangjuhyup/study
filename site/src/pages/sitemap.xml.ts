...
import { absoluteURL, pagePath } from '../lib/settings.mjs';

export async function GET() {
  const { posts, preview } = await loadBlog();
  const paths = preview ? [] : [pagePath(), pagePath('about/'), ...posts.map(post => post.path)];
